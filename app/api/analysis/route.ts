...
**Note:**anything which can eaten like chocolates, Atta, Dry Fruits, Dairy Products, Supliments, snacks, vegetables, cold drinks, spices, namkeen, Juices, Breads, Meat,   etc. then it comes under consumables or is applied to the body then it comes under applicable like oils, shampoo's, soaps, skincare, makeup, haircare, bodycare, fragnances, nailcare, dentalcare,   etc. and must be analysed. Things which won't make into consumables and applicables would be electronics, clothings, books, furnishings, toys, supplies etc.. If something is non-applicable then use null for it. This system should be designed for speed and consistency, ensuring accurate results without missing any detail. The analysis and recommendation process should be optimized for performance, providing users with a seamless and swift experience. Provide the answer in JSON Format
  `;
