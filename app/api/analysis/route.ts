...
export async function POST(req: NextRequest) {
  ...
  console.log(title)
  const prompt = `You are a master of ingredient analyzer and I am giving you a task. Show enthusiasm and give me a brief product description and its approximate calorie content along with the product category. Only if the product is a medicine then also provide me a brief description of its medicinal properties with medicine as the category. also, share the price of price of the product. In a highly efficient, consistent, and detailed manner, analyze the product title '${title} ' and provide a comprehensive analysis: The analysis should contain Product Overview(Present a brief product description and its approximate calorie content along with the product category. Only if the product is a medicine then also provide a brief description of its medicinal properties with medicine as the category.Ingredients Breakdown(Furnish an exact and detailed breakdown of all ingredients, including their amounts and units. You cannot miss even a single ingredient and include ingredients even with the smallest concentration. present it as array in the form of { ingredient_name: ingredient_amount || null, ingredient_name: ingredient_amount || null, etc...}), Harmful Ingredients and Side Effects(Include each and every harmful ingredients with a paragraph for the associated side effects of the ingredients to the human body and recommended daily limits for each of the ingredients.),Product Tags(Integrate relevant tags such as vegan, vegetarian, cruelty-free, paraben-free, sulphur-free, etc., according to the features of the respective product),Reviews Sentiment Analysis(Analyse the top 20 positive and top 20 negative reviews of the product and provide a collective sentiment score out of 100 and also summarize all 20 positive and all 20 negative reviews in a paragraph respectively),Safety Score Calculation(Efficiently calculate an overall safety score out of 100 using the following formula: safety_score_formula : Overall Safety Score = 100 - (Sum of Severity Points for Harmful Ingredients) - (5 * Number of Side Effects) + (2 * Number of Relevant Tags) + (1 * Number of Allergens) - (3 * Number of High-Risk Ingredients)), Alternative Recommendations products(Recommend three alternative product of indian market for the original product with safety scores. Please make sure that the alternatives have equal or higher safety scores than the original product. Provide a concise argument for each alternative). If you won't able to find ingredients of a particular product, try to fetch ingredients from any trusted websites. make sure that anything which can eaten like chocolates, Atta, Dry Fruits, Dairy Products, Supplements, snacks, vegetables, cold drinks, spices, namkeens, Juices, Bread, Meat,   etc. then it comes under consumables or is applied to the body then it comes under applicable like oils, shampoo's, soaps, skincare, makeup, haircare, body care, fragrances, nailcare, dental care, body cleanser,  etc. and must be analysed. Things that won't make it into consumables and applicable would be electronics, clothing, books, furnishings, toys, supplies, etc. If something is non-applicable then use null for it. This system should be designed for speed and consistency, ensuring accurate results without missing any detail. The analysis and recommendation process should be optimized for performance, providing users with a seamless and swift experience. please provide the answer in JSON Format, with alignments"
  `;
